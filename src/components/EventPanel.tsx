...
export default function EventPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      await load(); notify(`Checked ${result.checked} event sources · ${result.found} upcoming events found`);
  ...
  return <section className="events-page">
    <div className="events-hero"><div><span>RECRUITING CALENDAR</span><h2>Meet the companies before you apply.</h2><p>Upcoming recruiting sessions, engineering talks, workshops, conferences, career fairs, and hiring hackathons from official company calendars and student opportunity trackers.</p></div><button onClick={() => void refresh()} disabled={refreshing}><Icon name="refresh" className={refreshing ? "spin" : ""}/>{refreshing ? "Checking calendars…" : "Refresh events"}</button></div>
    <div className="event-metrics"><div><strong>{data.events.length}</strong><span>Upcoming events</span></div><div><strong>{thisMonth}</strong><span>This month</span></div><div><strong>{virtual}</strong><span>Virtual</span></div><div><strong>{data.directories.length}</strong><span>Official calendars</span></div></div>
    ...
    <div className="section-heading"><div><h2>Upcoming events</h2><p>Only future events are shown · dates use your local timezone · verify final details when registering</p></div><span>{visible.length} event{visible.length === 1 ? "" : "s"}</span></div>
    <div className="event-list">{loading ? <div className="event-empty">Loading company calendars…</div> : visible.length ? visible.map((event) => { const parts = dateParts(event.startAt); return <article className="event-card" key={event.id}><div className="event-date"><span>{parts.month}</span><strong>{parts.day}</strong></div><div className="event-body"><div><span className={`event-format ${event.format}`}>{event.format}</span><span className="event-category">{event.category.replace("-", " ")}</span><em>{daysUntil(event.startAt)}</em></div><h3>{event.title}</h3><strong>{event.company}</strong><p>{event.description}</p><div className="event-meta"><span><Icon name="clock"/>{timeRange(event)}</span><span><Icon name="pin"/>{event.location}</span><span><Icon name="file"/>{event.audience}</span></div></div><a href={event.registrationUrl} target="_blank" rel="noreferrer">View & register <Icon name="arrow"/></a></article>; }) : <div className="event-empty"><Icon name="calendar"/><h3>No dated events match right now.</h3><p>Refresh the calendars, adjust your filters, or open an official source below. New events appear automatically as companies publish them.</p></div>}</div>
