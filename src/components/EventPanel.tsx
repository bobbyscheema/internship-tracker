...
export default function EventPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const bayArea = data.events.length - virtual;
  ...
  return <section className="events-page">
    <div className="events-hero"><div><span>HOURLY RECRUITING RADAR</span><h2>Meet the companies before you apply.</h2><p>High-value virtual events you can join anywhere, plus in-person recruiting sessions, engineering talks, workshops, and career fairs close to UC Berkeley in the Bay Area.</p><small>{data.lastScrapeAt ? `Last checked ${new Date(data.lastScrapeAt).toLocaleString()}` : "First hourly scan pending"}</small></div><button onClick={() => void refresh()} disabled={refreshing}><Icon name="refresh" className={refreshing ? "spin" : ""}/>{refreshing ? "Checking sources…" : "Check now"}</button></div>
    <div className="event-metrics"><div><strong>{data.events.length}</strong><span>Upcoming events</span></div><div><strong>{thisMonth}</strong><span>This month</span></div><div><strong>{virtual}</strong><span>Virtual</span></div><div><strong>{bayArea}</strong><span>Bay Area</span></div></div>
    ...
    <div className="section-heading"><div><h2>Worth registering for</h2><p>Virtual anywhere · in person only near UC Berkeley and the Bay Area · dates use your local timezone</p></div><span>{visible.length} event{visible.length === 1 ? "" : "s"}</span></div>
