...
const EVENT_REFRESH_MS = 30 * 60_000;
export function ensureEventScheduler() {
  if (schedulerStarted) return;
  schedulerStarted = true;
  const last = getSetting("lastEventScrapeAt");
  if (!last || Date.now() - new Date(last).getTime() >= EVENT_REFRESH_MS) void scrapeEvents().catch(console.error);
  setInterval(() => void scrapeEvents().catch(console.error), EVENT_REFRESH_MS).unref();
}
